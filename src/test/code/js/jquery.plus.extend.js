...
jQuery.extend({
	...
	toggleFade : function(opt){
		...
		var speed = opt.speed;
		var auto;
		opt.auto != undefined ? auto = opt.auto : auto = true;
		var fadeClass = opt.fadeClass || "sel";
		var crlClass = opt.crlClass || "sel";
		var length = $(fadeElm).size();
		var curIndex = 0;
		var aimIndex = 0;
		var yn = true;
		fadeElm.eq(0).css({"opacity":1}).tabClass(fadeClass);
		crlElm.eq(0).tabClass(crlClass);

		var fade = function(){
			yn = false;
			if(curIndex != aimIndex){
				setTimeout(function(){
					fadeElm.eq(aimIndex).tabClass(fadeClass);
				},1200);
				...
				crlElm.eq(aimIndex).tabClass(crlClass);
				if(textElm && moveDtc){
